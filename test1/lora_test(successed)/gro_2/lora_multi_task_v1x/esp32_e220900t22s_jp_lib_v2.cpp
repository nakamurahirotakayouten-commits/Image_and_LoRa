...
int CLoRa::ReceiveFrame(struct RecvFrameE220900T22SJP_t *recv_frame) {
  ...
  // SerialMon.print("ReceiveFrameに来たよ");
  // SerialMon.print("before start."); //さぁどうかな？
  // int counter = 0;
  ...
  while (1) {
    ...
    // SerialMon.printf("\rnow searching : %0002d\n",counter); //この関数が回っているのかの確認
    // counter += 1;
    // if(counter > 1000){
    //   counter = 0;
    // }
    ...
    while (SerialLoRa.available()) {
      ...
      // SerialMon.print("データがきた"); //とりあえずデータを受信
...
int CLoRa::SendFrame(struct LoRaConfigItem_t &config, uint8_t *send_data, int size) {
  ...
  while (!LoRa_AUXPin); //はじめはこっちだったよ
  // while (!digitalRead(LoRa_AUXPin)); //これがいいんじゃないかって，言われた．by gpt これだとずっとループするみたい．だめだね．

  //  Print("\n.......... send data length = %d \n", size);
  SerialMon.println(transmission_method_type_val);
...
void CLoRa::SetDefaultConfigValue_into_private_val(){
    ...
    transmission_method_type_val = 0; // 9
...
int  CLoRa::ReceiveFrame(struct RecvFrameE220900T22SJP_t *recv_frame) {
  ...
  Serial.print("before start."); //さぁどうかな？
  ...
  while (1) {
    ...
    while (SerialLoRa.available()) {
      ...
      SerialMon.print("seriallora available now\n");
